...
- 安装本地 retry proxy
- 把 Antigravity 的 `jetski.cloudCodeUrl` 指向 `http://127.0.0.1:38475`
- 安装最新打包好的 retry status bar VSIX

如果你还想让它顺手尝试 reload 已打开的 Antigravity 窗口：
...
`./install.sh` 已经会自动修改 Antigravity 用户设置。最终生效的值是：
...
`./install.sh` 已经会把最新的 VSIX 安装到 `~/.antigravity/extensions`。

如果你更想走 Antigravity 里的手动安装路径：
